...
class SMALDataSource(BaseDataSource):
    """
    SMAL data source with coupled surface sampling.
    ...
    def __init__(self, cfg, *, modality_dims: Dict[str, int], device: torch.device):
        ...
        if meshes_cfg is None and mesh_path is None:
            raise ValueError("SMALDataSource requires data.meshes (dict) or data.mesh_path (single)")
    ...
    def _validate_shared_topology(self) -> None:
        ...
        for mod in mods[1:]:
            ...
            if faces.shape != ref_faces.shape or not np.array_equal(faces, ref_faces):
                raise ValueError("SMALDataSource requires identical faces across modalities")
